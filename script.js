...
const copy={
  ...
  ru:{eyebrow:"ЭСТЕТИЧЕСКАЯ МЕДИЦИНА",title:"Каталог продукции",subtitle:"Премиальная продукция для эстетической медицины, бренды и продуктовые линейки.",search:"Поиск продукции…",clickHint:"Нажмите на продукт, чтобы посмотреть детали",empty:"Ничего не найдено.",available:"Доступные форматы",packaging:"Упаковка",note:"Информация представлена для целей каталога и подбора продукции. Перед применением необходимо подтвердить актуальную документацию производителя, состав, назначение и регистрационный статус конкретного продукта для страны назначения.",productLine:"товарная позиция",productLines:"товарных позиций",formats:"форматов всего",footer:"Корейская эстетическая продукция · Глобальная дистрибуция",connect:"Контакты"}
};
...
grid.addEventListener("click",event=>{
  ...
  modalContent.innerHTML=`<div class="modalCat">${esc(catLabel(product.category))}</div><h2>${esc(product.name)}</h2>${variants}<div class="meta"><strong>${t("packaging")}:</strong> ${esc(localizePack(product.packaging)||"—")}</div><div class="description">${esc(lang==='ru'?(product.infoRu||product.info):product.info)}</div><div class="note">${t("note")}</div>`;
  modal.hidden=false;
});

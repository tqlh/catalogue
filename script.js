...
function formatInfo(value){
  ...
  return `${intro}<div class="keyDetails"><div class="keyDetailsTitle">KEY DETAILS</div><ul>${bullets.map(x=>`<li>${esc(x)}</li>`).join("")}</ul></div>`;
}
...
grid.addEventListener("click",event=>{
  ...
  const infoValue=lang==='ru'?(product.infoRu||product.info):product.info;
  modalContent.innerHTML=`<div class="modalGrid"><div class="modalText"><div class="modalCat">${esc(catLabel(product.category))}</div><h2>${esc(product.name)}</h2><div class="meta"><strong>${t("packaging")}:</strong> ${esc(localizePack(product.packaging)||"—")}</div></div><div class="modalImage"><img src="images/${slug}.jpg" alt="${esc(product.name)}" onerror="onProductImgError(this,'${slug}')"></div><section class="informationPanel"><div class="informationLabel">${t("overview")}</div><div class="description">${formatInfo(infoValue)}</div></section><div class="note">${t("note")}</div></div>`;
  modal.hidden=false;
